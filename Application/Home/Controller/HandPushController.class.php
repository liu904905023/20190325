...
//        exit();
    ...
    public function ylpush() {

        R("Base/getMenu");
        $this->display('yl_hand_push');
    }
    public function ylpushlist() {
        if(session(flag)==1&&session(servicestoretype)==1){
        ...
        if (IS_POST) {
            ...
            $url = C('SERVER_HOST') . "IPP3Order/IPP3So_MasterNotifyLogListYL";
    ...
    public function ylhandpush() {
        $data['Out_trade_no']    = I('Out_trade_no');
        $data['Transaction_id']  = I('Transaction_id');
        $data['systemUserSysNo'] = session('SysNO');
        $data['CustomerSysNo']   = session('servicestoreno');
        $data['Type']   ="银联";
        $url                     = C('SERVER_HOST') . "IPP3Order/IPP3HandPushJson";
        $list                    = http($url, $data);
        if ($list['Code'] == 0) {
            $return_info['Description'] = '推送成功！';
        } else {
