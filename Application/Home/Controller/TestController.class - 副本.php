...
//                            $this->reText( $data );
    ...
    private function CheckClear($SystemUserSysNO, $Openid) {
        $arr1 = array("SystemUserSysNO" => $SystemUserSysNO, "Openid" => (string)$Openid,

        );

        $url = C('SERVER_HOST') . "IPP3Order/IPP3TemplateMessageList";
        $info = json_encode($arr1);
        $head = array("Content-Type:application/json;charset=UTF-8", "Content-length:" . strlen($info));
    ...
    private function AddClear($CustomerServiceSysNO, $Openid, $SystemSysNO) {
        ...
        $clearinfo = array(
            ...
            "CustomerServiceSysNO" => $CustomerServiceSysNO, "Openid" => (string)$Openid, "SystemUserSysNO" => $SystemSysNO,
